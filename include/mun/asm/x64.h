...
MUN_INLINE word
asm_label_link_pos(asm_label* self){
  return self->pos - sizeof(word);
}
...
void asm_oper_init(asm_operand* self, asm_register reg);

typedef asm_operand asm_address;

void asm_addr_init(asm_address* self, asm_register base, asm_imm disp);
void asm_addr_init_scaled(asm_address* self, asm_register base, scale_factor scale, asm_imm disp);
...
// call
void asm_call_r(asm_buff* self, asm_register src);
void asm_call_a(asm_buff* self, asm_address* src);
void asm_call_l(asm_buff* self, asm_label* src);
...
void asm_jmp_l(asm_buff* self, asm_label* dst, bool near);
...
// Miscellaneous
void asm_j(asm_buff* self, condition cond, asm_label* dst, bool near);
void asm_bind(asm_buff* self, asm_label* label);
